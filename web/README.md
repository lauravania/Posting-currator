...
- **Rate limiting:** in-memory per-key limiter (`src/lib/rate-limit.ts`) on
  login, signup, upload, and analysis endpoints. Good enough for a
  single-instance MVP; swap for Redis/Upstash before scaling horizontally.
...
ContentPostImage, HashtagSet, Analytics, AIRecommendation`. Every
wedding-scoped or org-scoped table carries `organizationId` (directly, or
transitively via `weddingId -> Wedding.organizationId`) — see
`src/lib/db-scope.ts` for the enforcement helpers used by every server
action/route.
...
- Batch photo analysis runs synchronously in the request (fine for a
  30-photo MVP batch; move to a background queue for larger libraries or
